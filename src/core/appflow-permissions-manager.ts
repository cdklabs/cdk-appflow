    ...
    if (bucket) {
      ...
      if (isNew) {
        ...
        if (!addResult.statementAdded) {
          Annotations.of(bucket).addWarning('Cannot change bucket policy of imported S3 bucket. Ensure that your bucket policy contains appropriate permissions');
        }
      }
  ...
  public grantKeyEncrypt(key?: IKey) {
    if (key) {
      ...
      if (isNew) {
        ...
        if (!addResult.statementAdded) {
          Annotations.of(key).addWarning('Cannot change key policy of imported kms key. Ensure that your key policy contains appropriate permissions');
        }
      }
    }
  }
  ...
  public grantSecretRead(secret?: ISecret) {
    if (secret) {
      ...
      if (isNew) {
        ...
        if (!addResult.statementAdded) {
          Annotations.of(secret).addWarning('Cannot change secret policy of imported sm secret. Ensure that your secret policy contains appropriate permissions');
        }
